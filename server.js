...
schedule.scheduleJob('00 33 15 * * *', function(){
...
schedule.scheduleJob('00 43 00 * * *', function(){
			...
			if (count == length) {
				...
				if (varItem[mchNo].active) {
		            ...
		            if (varItem[mchNo].typeOfMachine == "detergent") {
		            	varItem[mchNo].totalManual = varItem[mchNo].totalManual + amount
		            	varItem[mchNo].manualPaid = varItem[mchNo].manualPaid + amount
		            	//(mchNo, cp, wp, mp, ap, tc, tw, tm, tp, type)
		            	mymqtt.updateMoney(mchNo, 0, 0, varItem[mchNo].manualPaid, 0, 0, 0, varItem[mchNo].totalManual, 0, "detManual")
		            	mymqtt.updateAccumulative(mchNo, "Manual_payment", amount)
		            } else {
		            	varItem[mchNo].manualPaid = varItem[mchNo].manualPaid + amount
		                mymqtt.updateMoney(mchNo, 0, 0, varItem[mchNo].manualPaid, 0, 0, 0, 0, 0, "Manual")
				...
				rpting.createEntry(mchNo, transId, varItem[mchNo].machineName, req.body.amount, req.session.username , timeNow, status, remark, myTransRecord)
				//rpting.save2csv("ePayment",myTransRecord[transId], rpting.uploadNothing)
				rpting.save2csv("manualPay",myTransRecord[transId], rpting.uploadNothing)
